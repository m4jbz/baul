# Este archivo/clase se encarga de la parte de iniciar sesión una vez run.py ya haya
# selecccionado la USB
...
class VentanaLogin(ctk.CTk):
    def __init__(self, baul_path, key_file_path):
        ...
        # Rutas del baul y del archivo llave
        self.baul_path = baul_path
        self.key_file_path = key_file_path

        self.title("Desbloquear Baúl")
        self.geometry("350x200") # Tamaño en px
        self.resizable(False, False) # NO es resizable
        ...
        # Aqui se escribira la contraseña
        ...
        self.pass_entry.bind("<Return>", self.intentar_login) # Hace que ENTER funcione como deberia

        # Botón desbloquear                                    - Función que se ejecuta al dar enter -
        self.login_button = ctk.CTkButton(self, text="Desbloquear", command=self.intentar_login, width=300)
        self.login_button.grid(row=2, column=0, padx=20, pady=10)

        # Etiqueta de texto que se usara cuando no se ingrese una contraseña
    ...
    def intentar_login(self, event=None):
        ...
        try:
            ...
            session_key = crypto.desbloquear_llave(password, vault_key_content)
            ...
            self.status_label.configure(text="No se encontró el archivo 'vault.key'.", text_color="red")
